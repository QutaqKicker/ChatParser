...
func BuildQuery[T Entity](request SelectBuildRequest) (string, []interface{}) {
	...
	queryBuilder.WriteString(buildSorter(request.Sorts))
	if request.Take != 0 {
...
func BuildUpdate[T Entity](values UpdateValues, filter any) (string, []interface{}) {
	if len(values) == 0 {
		panic("update parameters does not exists")
	}
...
func BuildDelete[T Entity](filter any) (string, []interface{}) {
	...
	whereString, values := buildWhere(filter, 1)
...
// RowsToEntities Переводит строчки из Rows в массив структур, тип которого указан в дженерике
