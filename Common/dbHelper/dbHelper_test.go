...
func TestBuildWhere(t *testing.T) {
	...
	for _, test := range tests {
		...
		if !queriesIsSame(query, whereStart+test.resultQuery) {
			t.Error(query, whereStart+test.resultQuery)
		}
		for i, param := range params {
			if param != test.params[i] {
				t.Error(params, test.params)
			}
		}
	}
}
...
func TestBuildSorter(t *testing.T) {
	...
	tests := []struct {
		...
		{emptySort, "created desc"},
		{singleSort, "id asc"},
		{tripleSort, "id asc, created asc, mapped_column desc"},
	}

	for _, test := range tests {
		query := buildSorter(test.sortFields)
		if !queriesIsSame(query, sortStart+test.result) {
			t.Error(query, sortStart+test.result)
		}
	}
}
...
func TestBuildDelete(t *testing.T) {
	...
	if !queriesIsSame(query, requiredQuery) {
		t.Error(query, requiredQuery)
	}
	if params[0] != filter.Id {
		t.Error(params[0], filter.Id)
	}
}
